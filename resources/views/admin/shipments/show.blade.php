            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <table class="table table-sm table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{$shipment->service->label}}</td>
                                    @can('admin')
                                    <td>{{$shipment->agent->name}}</td>
                                    @endcan
                                    <td>{{$shipment->gross_weight}} KG</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                            ...
                            <div class="card card-info border border-info" style="height:100%">
                                ...
                                <div class="card-body"  style="padding-bottom: 0;">
                                    <b>{{$shipment->shipper->name}}</b><br>
                                        ...
                                        {{ nl2br($shipment->shipper->street)}} <br>
                                        {{$shipment->shipper->city}} {{$shipment->shipper->zip}} <br>
                                        <b>{{$shipment->shipper->countryName}}</b><br>
                                        <i class="fas fa-phone-alt"></i> {{$shipment->shipper->phone}} <br>
                                        <i class="far fa-envelope"></i> {{$shipment->shipper->email}} <br>
                            ...
                            <div class="card card-info border border-info" style="height: 100%;">
                                ...
                                <div class="card-body" style="padding-bottom: 0;">
                                    <b>{{$shipment->receiver->name}}</b><br>
                                        {{ nl2br($shipment->receiver->street)}} <br>
                                       <b>City: </b> {{$shipment->receiver->city}} <b>Zip:</b> {{$shipment->receiver->zip}} <br>
                                        <b>State:</b> {{$shipment->receiver->state}}<br> 
                                        <b>{{$shipment->receiver->countryName}}</b><br>
                                        <i class="fas fa-phone-alt"></i> {{$shipment->receiver->phone}} <br>
                                        <i class="far fa-envelope"></i> {{$shipment->receiver->email}} <br>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col">
                            <b>Date: </b> {{$shipment->received_at->format('F d, Y')}}
                        </div>
                    </div>
                </div>
                <div class="card-footer">
                    <div class="row">
                        <div class="col-md-4">
                            <a href="{{route('print.shipments.receipt', $shipment->id)}}" target="_blank" class="btn btn-default btn-sm"><i class="fas fa-print"></i> Receipt</a>
                            <a href="{{route('print.shipments.transportCopy', $shipment->id)}}" target="_blank" class="btn btn-default btn-sm"><i class="fas fa-print"></i> Transport Copy</a>
                        </div>
                        <div class="col-md-4">
...
<div class="modal-body table-responsive">
    ...
    <div class="row">
       <form id="createNewUpdate" action="#" method="POST" class="col">
        @csrf
